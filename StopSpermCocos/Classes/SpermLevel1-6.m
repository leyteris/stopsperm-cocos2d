...
- (BOOL)ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event{
	self.touched = touch;
	if ( ![self headTouched:touch] && ![self tailTouched:touch] )   
    {  
        return NO;  
    }
	else if ([self headTouched:touch]) {
		self.ispressed = YES;
		[NSTimer scheduledTimerWithTimeInterval:0.3 target:self selector:@selector(istouched) userInfo:nil repeats:NO];
		//[self stopAction:moveAction];
		turntime = 2.0;
	...
	else if ([self tailTouched:touch]) {
		self.isTailPressed = YES;
		[self stopAction:moveAction];
	}
